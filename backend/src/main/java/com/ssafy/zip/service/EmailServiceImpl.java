...
public class EmailServiceImpl implements EmailService{
    ...
    public void sendVerifyMail(String email, String authToken) throws MessagingException, UnsupportedEncodingException {
        ...
        message.setSubject("[ZIP: 우리 가족의 또다른 집] 회원가입 이메일 인증"); // 제목

        String msgg="";
        msgg+= "<div style='margin:100px;'>";
        msgg+= "<h1> 안녕하세요 'ZIP: 우리 가족의 또다른 집'입니다. </h1>";
        msgg+= "<br>";
        ...
        String link = String.format("<strong><a href='http://k7a407.p.ssafy.io:8888/api/users/confirm-email?email=%s&authToken=%s'>인증하기</a>", email, authToken);
//        String link = String.format("<strong><a href='http://localhost:8080/api/users/confirm-email?email=%s&authToken=%s'>인증하기</a>", email, authToken);
    ...
    public void sendNewPasswordMail(String email, String newPassword) throws MessagingException, UnsupportedEncodingException {
        MimeMessage message = javaMailSender.createMimeMessage();
        message.addRecipients(MimeMessage.RecipientType.TO, email);  // 누구에게 보낼지
        message.setSubject("[ZIP: 우리 가족의 또다른 집] 임시 비밀번호 발급 안내"); // 제목
